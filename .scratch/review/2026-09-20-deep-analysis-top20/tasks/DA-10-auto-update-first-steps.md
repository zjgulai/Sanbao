...
- **未解释的不一致 → 已查明（同一窗口内补记）**：同一轮 CI 的 full 档同一判据却是 pass（3 个包）。
  ...
  **CI 的 Linux runner** 上重写结果不同 → 53 个跟踪文件被判「已修改」→ 后置的 `changed-packages`
  ...
  修法候选（用户 2026-09-22 拍板选 (a)，**已实现**）：(a) 门禁启动时**快照射程**，后置判据复用同一份；
  (b) 把这两个判据排到任何写盘判据之前（脆弱，将来加判据会再犯）；
  (c) `scripts-runnable` 改在临时副本里跑（最干净但最贵，且会改变该判据的语义）。

**已落地（2026-09-22，同一窗口）**：`scripts-gate` 侧在 `runGateChecks()` 之前调用
`takeChangedScope()` 落定射程，`changed-packages` / `permission-bits` 用 `readChangedScope()` 读回
（取不到时退回原路径）。端到端实测：跑到中途往包内产物写一字节，报告 `unstaged` 仍是写入前的 3、
命中包 0 个；同一写入走现算路径则立刻进射程（3→4）。决策记录见
[gate-scope-snapshot Note](../../../../docs/notes/implemented/contract/2026-09-22-gate-scope-snapshot.md)，
总账登记为 **P-55**（仪器读自己的尾气）。
