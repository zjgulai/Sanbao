...
- 优先级：P1
- 状态：`done`（2026-09-22：最低测试面与 LoopX 漏收修复已合入并推送；主树默认套件复验 11/11，新远端 CI 状态另见 DA-10）
- 依赖：无
- 估算：L
...
独立评审规格/质量均通过；2026-09-21 当时尚未合入主树。

2026-09-22 集成复核：修复随 `1734914` 合入，`4788320` 已推送至 Sanbao/main；
主树运行 `pnpm --dir packages/capabilities/dsh-loopx-plugin run test` 得到 11/11、exit 0，
同时覆盖两种后缀与真实子进程凭证隔离。本卡的最低测试面验收完成；新远端 CI 的完整读数
单独在 [DA-10](DA-10-auto-update-first-steps.md) 跟踪，不用本地结果代替远端结果。

**同日补充读数（可达性的全仓基线，本卡的射程外事项）**：门禁的包级测试是**显式点名式**，无自动发现——
`grep -o 'packages/…/test/*.spec.mjs' scripts/gate.mjs` 得 **9**（DA-07 接线后为 10），
而 `find packages -path '*/test/*.spec.mjs'` 得 **42**。即 **32 个包级 spec 不在门禁射程内**，
根仓 `test:gate` 只覆盖 `scripts/`，CI 也只跑 `scripts/gate.mjs`。本卡当初按「四包最低测试面」收口，
**不覆盖「全仓包级 spec 可达性」**；这条读数留档，别把它读成本卡已解决。
