...
- 优先级：**P0**
- 状态：`done`（2026-09-23：SEC-RT-001 接线 + 全表 runTier 核对 + 处置 A 落地——登记与执行路径一致）
- 依赖：无
- 估算：L
...
**裁决（2026-09-23，用户拍板 A）**：002/003/004/005/008/009 六条 `runTier` 改
`'full'`；001/003A/006/007/011 保持 `'quick'`（各有无条件 quick 入口）。登记表
头注释落语义定义（quick=判据无条件点名；full=条件+full 覆盖，跨包 gap 接受）。
验证：security-contract-master 审计 11/11 passed + selftest 6/6 绿。
本卡可收口：SEC-RT-001 接线落地、全表 11 条 runTier 与执行路径一致。
