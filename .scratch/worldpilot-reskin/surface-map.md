...
| S12 | 平台色彩系统 | 暗 `#58B848` / 亮 `#347A2F`，15 预设 × 8 色卡 | `--wp-*` 单一源 + `--dsw-alias-*` 派生；**1 套品牌 × 明暗双色道**，codex/graphite/midnight 降 legacy | `packages/platform/dsh-theme-local/src/{client/theme-tokens.ts,client/accent-swatches.ts,client/presets.ts,theme-settings.ts}` + `shared/client/lute-tokens.ts`→`--wp-*` | `theme-tokens`（`NAMESPACE` 扩含 `wp`）、`theme-tokens-baseline-frozen`（只减不增）、`presets.test.ts`、`accent-swatches.test.ts`、`theme-tokens.test.ts` |
...
| `theme-tokens` | `NAMESPACE='--(?:dsw\|ds\|dsh)-'` | 扩为含 `wp`，**同一次提交**建 `--wp-*` 定义之家 | S1 |
