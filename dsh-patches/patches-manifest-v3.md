...
| upstream-brand-surface 批二 | **平台面清扫（PWA 清单 / favicon / 窗口标题 / CA 名 / 系统提示 / CLI / 徽章技能）** | `dsh-web-frontend/dist/manifest.webmanifest`（name/short_name）、同 dist `favicon.svg`（换 `brand/logo/sanbao-favicon.svg`）、`lib/src-*.js`（windowTitle）、`lib/lan-https-certificate-*.js`（CA_COMMON_NAME）、`@agents-anywhere/dsh-bridge-next`（用户可见串）、`dsh-system-prompt`/`dsh-web-app`/`dsh-app-boot`（系统提示）、`dsh/lib/bin.js`（CLI 描述）、`dsh-skill-badge`（provider 返回空清单=停用）、profile 影子副本 layout/settings-models | 上游名 **0 处**（8 条：5 `ck` + 3 `ckn`）+ `apply-extras.sh --check` 全绿 | 装配期 `dsh-patches/upstream-brand-surface/apply-extras.sh apply --staged`（幂等）；装机 app 必须运行中打。**旧 CA 状态目录改名**（`lan-https` → `.orig-deepseek-ca`）以新 CN 重签 CA——须所有设备重新信任 | 模型/提供方真名 `DeepSeek` 与 `@deepseek-ai/*` 包 id 仍不改 |
