...
## 5. 增量登记（2026-09-19，管理层 preset 整合轮 · ADR-0129 D9/D6④）

| # | 债务 | 现状 | 偿还方向 | 触发条件 |
|---|---|---|---|---|
| D-19-1 | **T0 通用技能含租户专属工具挂全部 50 执行岗**：`xindaya-translator`、`kami` 是租户专属，T0 判据「任何岗位都用得上」对其不成立（research/16 §3.4）。MGT 3 岗已按 ADR-0129 D5 剔除；AGT 50 岗未动（动 T0_NAMES = 50 个全量重生成 + 三线名单门禁联动，爆炸半径大） | 已登记，未偿还 | 在 T0 判据里增设「租户专属工具不入 T0」细则并评审这两条的去留（降 T1 按岗位族挂，或迁出租户工具线）；改动走 build-generic-manifest.mjs 的 T0_NAMES 唯一家 | 下一次技能三线评审，或租户工具投诉/误挂实据出现 |
| D-19-2 | **岗位数量断言的彻底单一事实源化**：本轮已把 generate.mjs 断言改为「材料 role_count 派生 + 显式常量对照」（ADR-0129 D6④），但 shipped-presets `expectPatternCount`、verify-lossless 覆盖计数、live-presets 账本 presetCount 仍是各自维护的派生面——同一事实多个家（research/16 §3.6） | 最小改已落，彻底源化未做 | 让各处计数从材料 catalog 的 role_count 单一来源派生（或互相交叉断言），消除人工同步点 | 下一次岗位编制变更（材料 role_count 变动）前必须完成，否则变更即五处手改 |
