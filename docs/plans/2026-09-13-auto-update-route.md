...
| 拿到 Developer ID | ③ 换签名身份 + 公证 + 手册删掉「右键打开」；④ 这一版显式引导重授三项 TCC | 证书 | 未决：**Developer ID 采购是本计划唯一的外部前置**（见「未决」一节） |
...
- **Developer ID 采购（唯一的外部前置）**：Apple Developer Program 年费账号 + Developer ID
  Application 证书；换身份的代价（③④ 那一版用户重授三项 TCC + 客户通告文案）在 §0 已量化。
  这一项**不在工程侧排期**，是采购决策。
