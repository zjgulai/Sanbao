...
- 日期：2026-09-13
- 状态：**部分完成**（§5 的「现在」行两步 ①② 已于 2026-09-21 落地，见 [ADR-0151](../adr/ADR-0151.md) /
  [ADR-0152](../adr/ADR-0152.md)；③④⑤⑥⑦⑧ 仍未开工，外部前置 = Apple Developer ID Application 证书）
- 相关：ADR-0063（固定自签身份与 TCC 授权延续）、ADR-0058（入库清单）、ADR-0067（产物不可删除）、ADR-0065（替换运行中 app bundle 会白屏）、ADR-0151（feed 契约）、ADR-0152（更新器骨架边界）、`docs/sop/dmg-release.md`
...
| 阶段 | 做什么 | 依赖 | 状态 |
| --- | --- | --- | --- |
| 现在 | ① `latest.json` 生成 + 归档 + 门禁核对；② 更新器骨架（只做「检查 + 提示有新版」，不做安装） | 无——**不白做**：这两步在人工交付阶段就有价值（用户能看到有新版本） | **✅ 已完成（2026-09-21）**：① 契约与判据 [ADR-0151](../adr/ADR-0151.md)；② 包 `packages/platform/dsh-update-local` [ADR-0152](../adr/ADR-0152.md)。都在等 Developer ID 的部分之外，零前置 |
| 拿到 Developer ID | ③ 换签名身份 + 公证 + 手册删掉「右键打开」；④ 这一版显式引导重授三项 TCC | 证书 | **挂起（2026-09-21 决策：暂不采购，维持自签）**——重启条件见「未决」 |
| 换身份之后 | ⑤ 更新器接上「下载 + 校验 + 调 install.sh + 重启」；⑥ 通道与回滚 | ③④ 完成 | 未开始（依赖 ③④） |
| 之后 | ⑦ `latest.json` 的 Ed25519 签名；⑧ 评估是否需要 app 本体增量更新（路线 A） | 运行一段时间后的实际需求 | 未开始 |
...
- **Developer ID 采购（唯一的外部前置）：用户 2026-09-21 拍板「暂不采购，维持自签」。**
  含义：人工交付继续（DMG + 手册），更新器停在「检查 + 提示有新版」（①② 已落地那一版），
  不做任何依赖 Developer ID 的部分；③④⑤⑥⑦ 全部挂起。代价如实记：换身份的时点越晚，
  需要重授三项 TCC 的用户面越大（§0 已量化）——本决策**不是「不做」，是「先不做」**，
  重启条件是出现外部客户或跨机分发需求。
- ①② 落地后，「检查 + 提示」链路已可用；当前 `releases/latest` 上还没有 `latest.json` 附件
  （机制后第一版才会带上），所以实机读数是 `feed-unreadable（HTTP 404）`——如实报「读不到」，
  不是「已是最新」。
- 加签名（⑦）与自动安装（⑤）都不提前做：前者要 Developer ID 与私钥管理决策，后者会把
  Gatekeeper 问题推给用户（见上）。
