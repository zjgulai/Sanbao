...
| 本次增量 | `v2.3.3..HEAD` = **18 个提交** + 3 个新提交（门禁修复 / 换底 / 判据与留痕） | 见 §1 |
...
| 本机运行的 DSH | **有**（`dsh-running.sh` 退出码 0，pid 96890） | **必须先退出**，否则白屏红线 |
...
| 读数 | 值 | 出处 |
| --- | --- | --- |
| 2.3.3 出货技能面 | **349**（`p2s-*` 164 + 非 p2s 185） | 卷内 `skills-presets.tar.gz` 实测 |
| `skill-map.json` 的 `p2s-*` 引用 | 164，与出货面 `p2s-*` **完全相等**（无一多余、无一缺失） | 两向比对 |
| 新接入的 52 张卡 | **均不在**出货面（`skill-map` 命中 0/52，preset 命中 0/52） | ⇒ **本版出货面预计仍为 349** |
| `packaging/shipped-skills.json` 产品级白名单 | 15 条 | ADR-0074 |

**这意味着**：换底（1338 → 1390）是**产品侧底本**的收敛，**不改变对客户的出货面**。
若装配后技能数与 349 不符，**先查原因再继续**——不是「预期内的波动」。
