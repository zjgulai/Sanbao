...
test('S2 验证器判红：必须红，且把它的原文输出带出来', () => {
  const stub = stubRun({ 'verify-generic.mjs': '✗ 通用技能线未达标：T0 挂载缺口 3 个岗位' })
  const r = checkSkillLines({ repoRoot, runOne: stub })
  stub.assertAllHit()
...
test('S5 无摘要行：判绿但读数缺失必须说出来，不许静默当成通过', () => {
  const r = checkSkillLines({ repoRoot, runOne: stubRun({}, { emptyStdout: true }) })
  assert.equal(r.passed, true)
  assert.match(r.note, /摘要行未能解析（读数缺失，不是通过）/, '读数死了但判据还绿 = P-02，必须显式说明')
})

test('S6 恒真桩突变：把 passed 钉成常量 true，上面几条必须失效', () => {
  ...
  const red = checkSkillLines({ repoRoot, runOne: stub })
  stub.assertAllHit()
  const green = checkSkillLines({ repoRoot, runOne: stubRun() })
...
test('S7 包内契约测试在射程内：新增一条会红的 spec，本项必须跟着红', () => {
