...
#### 汉译时「围栏里译不译」的口径（2026-09-15 立）

译者是模型，逐条判断会漂。这里给一条可执行的分界与两张例外清单，**新技能按它判，不再逐批重议**。

| 围栏形态 | 处置 | 为什么 |
| --- | --- | --- |
| 带语言标签（`bash`/`json`/`sql`/`python`/`yaml`…） | **逐字节保留** | 是要能跑的东西，不是要读的东西 |
| 无标签 / `text`，内容是代码、命令行、路径、语法规格、配置片段 | **保留** | 同上；把 `<type>[optional scope]: <description>` 译成中文就不再是 Conventional Commits 的语法了 |
| 无标签 / `text`，内容是**散文**：决策树问题、检查清单、流程图里的词、字段说明 | **就地汉译** | 读者读的是这句话的意思。基线先例见 `simplify-codebase.body.md` 的 `text` 模板整段中文 |
| 无标签，内容是**含注释的代码/目录树** | **只译注释，不动代码** | 如 `sources/  Raw data definitions` → `sources/  原始数据定义` |
| 无标签，是 **ASCII 对齐图**（框线、箭头列） | **保留原文，不译** | 中英文宽度不同，译了框线就散；要译得整张重排，属重写不属翻译 |
| 无标签，是**技能将要输出给用户的话** | **保留原文** | 它是指令的一部分——译了等于改变 agent 打印什么，那是行为变更不是排版 |

⛔ **别把「看起来是英文散文」直接当结论。** 2026-09-15 本批实测过一遍：机械判据把 74 处无标签围栏分成「码味 31 / 散文 43」，逐条复核后**散文那一桶里约一半其实是**工具调用片段（`take_snapshot(verbose: true)`）、ASCII 对齐图、`.env` 注释、示例内容——它们全都属上表的「保留」侧。真属「该译」的量级是 **20 处上下**，且分散在 18 条技能里。

⚠️ **本条的射程**：翻译源在 `staging/translations/`（`.gitignore` 掉、**不入库**），所以「围栏口径统一」这类改动**在仓库里只留下一组新的 sha256**，没有任何可评审的内容 diff。要改就得整批重装（`intake-install.mjs --skills a,b,c --force`），改完的价值体现在 `~/.dsh/skills/` 与模型的读数上，评审面则是本表这条口径本身。**口径入库、批量改译留待需要时再做**——这是有意的取舍，不是漏做。
