...
description: "纸感排版系统：用 Kami 模板产出简历、一页纸、白皮书、长文、信件、作品集、个股研报、更新日志与幻灯片（HTML/PDF/PPTX/Marp），也做可部署的多语言落地页。触发词：做 PDF、排版、简历、一页纸、白皮书、作品集、研报、更新日志、一套幻灯片、落地页、Kami。何时不用：要印刷级通用 PDF 版面用 minimax-pdf；单文件网页翻页 PPT 用 guizang-ppt-skill；可交互图表与整页 HTML 报告用 lieflat-charts；审计或改造已有产品站点不适用。安全边界：不虚构数据与素材，缺料标 [DATA NEEDED]；中文正文字体仓耳今楷02 商用需另行授权。"
enabled: "true"
user-invocable: true
metadata:
  source: "third-party"
  upstream: "https://github.com/tw93/Kami"
  upstream_commit: "6b39d04567034a0b14671da3dbcf65e370dda9a8"
  version: "1.15.0"
  batch: "kami-github"
  license: "MIT（代码与模板）；仓耳今楷02 字体商用需单独授权"
  classified_by: "dsh-user-skill-install"
---
