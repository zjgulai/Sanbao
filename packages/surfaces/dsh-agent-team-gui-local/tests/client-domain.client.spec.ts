...
describe('client domain validation and view models', () => {
  ...
  it('keeps responsive, reduced-motion, and theme styles scoped to plugin roots', () => {
    ...
    expect(CLIENT_STYLES).toContain('transition:color 180ms ease,background-color 180ms ease,border-color 180ms ease,opacity 180ms ease,transform 180ms ease')
    expect(CLIENT_STYLES).toContain('var(--dsw-alias-state-business-primary)')
    expect(CLIENT_STYLES).toContain('@media(prefers-reduced-motion:reduce){.atg-page *')
    expect(CLIENT_STYLES).toContain('.atg-recipe-layout,.atg-backup-layout{display:grid;grid-template-columns:minmax(0,1.1fr) minmax(280px,.9fr)')
    expect(CLIENT_STYLES).toContain('.atg-validation-footer{position:sticky')
    expect(CLIENT_STYLES).toContain('.atg-validation-footer{position:sticky;bottom:0;z-index:1;display:flex;align-items:end;justify-content:flex-end;gap:8px;margin-top:2px;padding:13px 0 1px;border-top:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1)}')
    expect(CLIENT_STYLES).not.toContain('linear-gradient')
    expect(CLIENT_STYLES).toContain('.atg-backup-preview.is-danger')
    expect(CLIENT_STYLES).toContain('@media(max-width:430px){.atg-validation-footer')
