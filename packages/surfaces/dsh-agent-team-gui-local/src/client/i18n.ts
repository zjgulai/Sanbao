...
const zh = {
  ...
  loading: '正在读取小队…', reconnecting: '小队服务暂时不可用，正在重新连接。', reconnectDetail: '你的配置仍保存在本地。连接恢复后会自动重新读取。', incompatibleHost: 'Agent Team GUI 前后端版本不一致。请重启三宝，然后刷新页面。',
  requestTimeout: '请求超时。控件已恢复，可以重试；小队服务恢复后也会自动重新连接。',
  slotCrashed: '小队界面遇到不兼容的数据', slotCrashedDetail: '三宝主界面仍可继续使用。请重试；若问题持续，请重启 Host 后刷新页面。',
  ...
  templateNoteDevelopment: '对请求的变更做计划、实施与评审，明确验收标准并附带回归检查。',
  templateNoteReview: '独立进行正确性、安全与测试评审，保留分歧交给主代理决策。',
  templateNoteProduct: '平衡用户价值、交互质量、可访问性与实现可行性。',
  templateRoleDevelopmentPlanner: '分析请求、约束、风险与验收标准，产出可执行的依赖感知计划。',
  templateRoleDevelopmentImplementer: '完整实现请求的变更，保持既有行为不回退，并验证结果。',
  templateRoleDevelopmentReviewer: '评审正确性、安全、UX、回归与缺失测试，回报有证据的发现。',
  templateRoleReviewCorrectness: '评审行为与逻辑正确性，只报有证据的发现。',
  templateRoleReviewSecurity: '评审信任边界、密钥、权限、注入与破坏性行为。',
  templateRoleReviewTests: '运行或设计聚焦测试，识别回归风险与覆盖缺口。',
  templateRoleProductProduct: '澄清用户问题、成功指标、优先级与边界情况。',
  templateRoleProductUx: '设计清晰的交互流程、信息架构、状态与可访问的微文案。',
  templateRoleProductEngineer: '评估可行性、架构、风险，并给出可验证的交付计划。',
...
const en: Record<keyof typeof zh, string> = {
  ...
  loading: 'Loading teams…', reconnecting: 'The team service is temporarily unavailable. Reconnecting…', reconnectDetail: 'Your configuration remains stored locally and will reload after reconnecting.', incompatibleHost: 'Agent Team GUI client/host versions do not match. Restart Sanbao, then refresh the page.',
  ...
  slotCrashed: 'The team view received incompatible data', slotCrashedDetail: 'The rest of Sanbao is still usable. Retry here; if it persists, restart the Host and refresh the page.',
  ...
  templateNoteDevelopment: 'Plan, implement, and review the requested change with explicit acceptance criteria and regression checks.',
  templateNoteReview: 'Run independent correctness, security, and test reviews, preserving disagreements for the lead Agent.',
  templateNoteProduct: 'Balance user value, interaction quality, accessibility, and implementation feasibility.',
  templateRoleDevelopmentPlanner: 'Analyze the request, constraints, risks, and acceptance criteria. Produce an executable dependency-aware plan.',
  templateRoleDevelopmentImplementer: 'Implement the requested change completely, preserve existing behavior, and verify the result.',
  templateRoleDevelopmentReviewer: 'Review correctness, security, UX, regressions, and missing tests. Return evidence-backed findings.',
  templateRoleReviewCorrectness: 'Review behavior and logic for correctness. Report only evidence-backed findings.',
  templateRoleReviewSecurity: 'Review trust boundaries, secrets, permissions, injection, and destructive behavior.',
  templateRoleReviewTests: 'Run or design focused tests and identify regression risk and missing coverage.',
  templateRoleProductProduct: 'Clarify the user problem, success metrics, priorities, and edge cases.',
  templateRoleProductUx: 'Design a clear interaction flow, information architecture, states, and accessible microcopy.',
  templateRoleProductEngineer: 'Assess feasibility, architecture, risks, and a verified delivery plan.',
