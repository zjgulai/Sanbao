...
抽屉里有**两个分区，两份契约**（[ADR-0062](../../../docs/adr/ADR-0062.md)）：

| 分区 | 一张卡是什么 | 点开做什么 | 可能失败吗 |
| --- | --- | --- | --- |
| **产品** | 本机已产品化的一个 Agent（`product.json`） | 开它的面板，或新建会话并选中岗位 | 会——三级判据的最后一级是**禁用并写明原因** |
| **业务系统** | 路特主域名下的一个系统入口（31 个） | 交给**系统默认浏览器**打开 | 不会「本机没装」，只有「无地址」或「宿主半边没加载」 |

两个分区不共用一个开卡联合类型，也不共用一种卡片：一个按钮不能有时开本应用的面板、有时把你带离本应用。
...
> **卡的主键是产品**，一产品一卡（ADR-0045 R4）。工作目录降为卡上的元信息（会话在那里打开）。

没有 `product.json` 的目录**不再是卡片**（ADR-0045 取代 ADR-0028 与 ADR-0033 第 6 条）：
未产品化的数量只以页脚**一行小字**报出，不占卡位。「我扫了 25 个目录、没有一个声明产品」
这个事实仍然有答案，只是不再花 24 张卡来讲。
...
标志的语义是**「挂起 ≠ 失败」**（[ADR-0153](../../../docs/adr/ADR-0153.md)）：放置走 rAF，而窗口不可见时 Chromium 冻结 rAF（合盖/锁屏/窗口被盖住），此时入口行不在场是**暂停**不是故障——宽限期到点若 `document.hidden` 就不判也不轮询，恢复可见再给一档宽限；
已经挂上的标志**必须随条件消失**（迟到的放置归位即清除，否则每个探针都读到一份陈旧假警）；只有「可见 + 宽限过后仍无入口行」才响亮降级。
...
## 业务系统分区：本包唯一拥有的数据，和唯一的动作

**这个包以前不拥有任何数据。现在它拥有一份** —— 这是 [ADR-0062](../../../docs/adr/ADR-0062.md) 记下的身份变化，不是附带效果。
它落在 `src/catalog/` 的**三份文件**里，三个家互不覆盖：

| 文件 | 谁写 | 是什么 |
| --- | --- | --- |
| `systems.json` | `pnpm run sync:systems`（机器） | 身份：31 个系统的中英文名、简介、类型、标签、动作文案、地址、图标词汇。必须**幂等**——重跑不改一个字节，好让 `git diff` 就是「站点变了什么」 |
| `role-map.json` | **人** | 岗位归属：`primary`（决定分组，一系统只出现一次）+ `also`（卡上 chip，且参与搜索）+ 一条 `why`。**永不自动覆盖** |
| `reachability.json` | `pnpm run probe:systems` | 带日期的巡检读数：可达性 + 入口是否就是登录页 |

为什么 catalog 是**打包**进产物而不是运行期读文件：实测装载点包只带 `lib/` + `cordis.patch.yml` + `README.md`，
运行期 `readFileSync('../catalog/…')` 会指向不存在的路径，而失败模式是「profile 同步后抽屉里出现一个空分区」。
代价是宿主半边体积 8 KB → 71 KB。

**面板打开时零网络请求。** 可达性只在离线脚本里测，结果带日期进 catalog，卡片把它读作「某日的读数」而不是「此刻可用」。

### 外链为什么必须走宿主

实测：本机壳的主进程对 http/https 的 `target="_blank"` 一律 `deny`（P0-6v2，只放行 `mailto:`）。
所以 `<a target="_blank">` 在这个壳里**点了没有任何反应**——没有浏览器侧兜底可依赖。卡片因此 POST 一个
**slug** 给宿主，由宿主解地址并启动系统默认浏览器（darwin `open` / win32 `rundll32 url.dll,FileProtocolHandler` / linux `xdg-open`，argv 传参、不经 shell）。

> **客户端永远不传地址。** 路由的输入词汇就是 catalog 的键，未知 slug 是 404。这条一旦松动，这个路由就变成
> P0-6v2 特意移除的「打开任意 URL」原语——`tests/systems-routes.spec.ts` 有用例守着它。

### 岗位分组

主岗位决定分组（顺序沿用 role-matrix 的两层分类：平面 → 责任域 → 岗位编号），兼属岗位是卡上的 chip。
**roster 里没有的岗位照常渲染并标记**，不隐藏：隐藏会让一次 preset 改名看起来像系统被删了。
页脚如实报覆盖率（当前 31 个系统覆盖 23/50 个岗位，其余岗位在这批系统里确实没有入口）。

## 契约

- host 面注册**四条** exact 路由：`/api/dsh-newapp/health`（加载探测器）、
  `/api/dsh-newapp/products`（`product.json` 发现）、`/api/dsh-newapp/systems`（业务系统 catalog）、
  `/api/dsh-newapp/open-system`（**唯一会动的路由**：POST 一个 slug，宿主交给系统浏览器打开）。
  四条都过**共享信任栅栏**（`shared/host/pair-access.ts`：回环 + 已配对设备）。
