...
export interface NativeSidebarRight {
  ...
  active(): { readonly kind?: string } | undefined
}
...
/** `ctx.get` 的窄面：本包读的两个服务名都不在本包的类型表里。 */
 ...
 */
export function nativeTabRegistry(ctx: ClientContext): NativeTabRegistry | undefined {
  // ctx.get 的键是各包声明合并出来的，本包没引 sidebar-right 的类型，所以键
  // 不在表里；这里的转型只为过类型，读到的仍是 cordis 的真实服务。
  return (ctx as unknown as ServiceReader).get('sidebarRightTabs') as NativeTabRegistry | undefined
}
 ...
 */
export function nativeRightbar(ctx: ClientContext): NativeSidebarRight | undefined {
  return (ctx as unknown as ServiceReader).get('sidebarRight') as NativeSidebarRight | undefined
}
 ...
 */
export function nativeSlots(ctx: ClientContext): NativeSlots {
  // ctx.slots 的类型来自 ui-slots 的 SlotMap，而 'sidebar.right.pane.tab' 由
  // sidebar-right 包声明合并进来；本包没引它，键不在表里。窄化到本包用到的
  // 两个方法，比在每个调用点写 as any 更收敛。
  return ctx.slots as unknown as NativeSlots
}
